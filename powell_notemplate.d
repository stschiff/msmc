...
alias double delegate(const double[]) PowellMinfunc_t;

class F1dim {
  const double[] p, xi;
  int n;
  PowellMinfunc_t func;
  double[] xt;

  this(in double[] p, in double[] xi, PowellMinfunc_t func) {
    this.p = p;
    this.xi = xi;
    n = cast(int)p.length;
    this.func = func;
...
class Linemethod {
  ...
  this(PowellMinfunc_t func) {
    this.func = func;
  }
  
  double linmin() {
    stderr.writeln("enter linmin");
    writeln(xi.length, " ", p.length);
    double ax,xx,xmin;
    n=cast(int)p.length;
    auto f1dim = new F1dim(p,xi,func);
    ax=0.0;
    xx=1.0;
...
class Powell : Linemethod {
  ...
  double ftol;
  bool verbose;
  this(PowellMinfunc_t func, bool verbose=false, double ftol=3.0e-8) {
    super(func);
    this.ftol = ftol;
