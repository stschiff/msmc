...
## Consensus calling
### ...from BAM
I assume that you have one bam file for each sample you want to study. You will need a reference file for this. You can now call the consensus sequence for each individual via `tools/conensus_caller.sh`, for example:

    tools/consensus_caller.sh -r <reference.fa> -c <chr> -d <depth> -o <output_prefix> <bam_file>
...
When your data was generated by Complete Genomics, you need to have access to the vcfBeta-file, as described in their [File Format Documentation](http://www.completegenomics.com/customer-support/documentation/100357139.html). You can call the consensus sequence via:
    ...
    tools/consensus_caller.sh -r <reference.fa> -c <chr> -o <output_prefix> <cg_vcfBeta-file>
...
When having generated the vcf- and mask-files and having phased as many heterozygotes as possible, you can generate an input file via:
    ...
    tools/generate_multihetsep.py --mask <mappability_mask.bed.gz> <vcf_1.gz> <mask_1.bed.gz> <vcf_2.gz> <mask_2.bed.gz> ...

where you should use so many vcf/mask pairs as you want individuals to be studied simultaneously.
The mask file `<mappability_mask.bed.gz>` is optional. You can give any additional mask in gzipped bed format for that argument, and it is recommended to put the mappability mask, e.g. generated with Heng Li's programs as described here: [SNPable](http://lh3lh3.users.sourceforge.net/snpable.shtml). For the human reference (hs37) I provide mask files for every chromosome on the same FTP site as given above.
